...
public class CopyOfDescriptionNodeCreator {
	...
	public CopyOfDescriptionNodeCreator() {
	...
	private List<Element> getDescriptions(String text) {
		...
		for(String token : tokens) {
			if(!token.trim().isEmpty()) {
				if(token.trim().matches("(\\p{Punct})+")) {
					for(String t : token.trim().split("\\B"))
						if(!t.isEmpty())
		...
		List<String> normalizedTokens2 = new LinkedList<String>();
		for(String token : normalizedTokens) {
				if(token.startsWith(".")) {
					String previous = normalizedTokens2.remove(normalizedTokens2.size() - 1);
					normalizedTokens2.add((previous + token).trim());
				} else
					normalizedTokens2.add(token.trim());
		}
		...
		for(String token : normalizedTokens2) {
	...
	public static void main(String[] args) throws JDOMException, IOException {
		...
		for(File inputFile : input.listFiles()) {
			...
			CopyOfDescriptionNodeCreator creator = new CopyOfDescriptionNodeCreator();
